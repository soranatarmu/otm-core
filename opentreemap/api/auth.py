...
def decodebasicauth(strg):
    if strg is None or not len(strg):
        return None
    else:
        m = re.match(r'([^:]*)\:(.*)', base64.encodebytes(strg).decode())
        if m is not None:
...
def parse_basicauth(authstr):
    firstmatch("Basic b'(.*)'", authstr)
    parsed_binary_auth_as_string = firstmatch("Basic b'(.*)'", authstr)
    if parsed_binary_auth_as_string is None:
        return None

    auth = decodebasicauth(parsed_binary_auth_as_string.encode())
    if auth is None:
        return None
    else:
